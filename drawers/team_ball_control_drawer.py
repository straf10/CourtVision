...
class TeamBallControlDrawer:
    ...
    def draw_frame(self, frame, frame_num, team_ball_control):
        ...
        # Draw rectangle
        cv2.rectangle(overlay, (rect_x1, rect_y1), (rect_x2, rect_y2), (255, 255, 255), cv2.FILLED)
        alpha = 0.8
        ...
        team_1_percentage = team_1_num_frames / total_frames if total_frames > 0 else 0
        team_2_percentage = team_2_num_frames / total_frames if total_frames > 0 else 0

        cv2.putText(frame, f"Team 1: {team_1_percentage*100:.2f}%", (text_x, text_y1), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), font_thickness)
        cv2.putText(frame, f"Team 2: {team_2_percentage*100:.2f}%", (text_x, text_y2), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), font_thickness)
        return frame